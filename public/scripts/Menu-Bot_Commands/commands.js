  ...
  try {
      ...
      const commandCategories = {
        ...
        "Lệnh Thành viên": {
          "Thông tin & Tiện ích": [
            ...
            // Thêm cứng các lệnh !plan
            {
              name: "plan",
              description:
                "Giao việc cho thành viên, có deadline, tự động nhắc nhở",
              usage: "[nội dung] [dd/mm/yyyy hh:mm] @user",
              icon: "📅",
              countdown: 5,
              type: 1,
              permission: "all",
    ...
    socket.on("error", (error) => {
      ...
      commandList.innerHTML =
        '<div class="error">Đã xảy ra lỗi khi tải danh sách lệnh. Vui lòng thử lại sau.</div>';
    });
  } catch (error) {
    ...
    for (const [subCategory, commands] of Object.entries(subCategories)) {
      ...
      filteredCommands.forEach((command) => {
        ...
        commandElement.innerHTML = `
                    ...
                    ${
                      command.alias
                        ? `<p class="alias"><strong>Tên gọi khác:</strong> !${command.alias.join(
                            ", !"
                          )}</p>`
                        : ""
                    }
                    <p class="countdown"><strong>Thời gian chờ:</strong> ${
                      command.countdown
                    }s</p>
                `;
