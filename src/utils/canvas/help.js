...
// Tạo Hình Lệnh !Help
export async function createInstructionsImage(
  helpContent,
  isAdminBox,
  width = 800
) {
  ...
  for (const key in helpContent.allMembers) {
    if (helpContent.allMembers.hasOwnProperty(key)) {
      ...
      const lineWidth =
        labelWidth + space + ctxTemp.measureText(valueHelpContent).width;
      if (lineWidth > width) {
        yTemp += 52;
      }
      yTemp += 52;
  ...
  yTemp += 200; // Tăng khoảng cách dưới để đủ chỗ cho nhiều lệnh hơn

  const height = yTemp > 1000 ? yTemp : 1000; // Tăng chiều cao tối thiểu lên 1000
  ...
  for (const key in helpContent.allMembers) {
    if (helpContent.allMembers.hasOwnProperty(key)) {
      ...
      const valueHelpContent = " -> " + helpContent.allMembers[key].description;
      const lineWidth =
        labelWidth + space + ctx.measureText(valueHelpContent).width;
      if (lineWidth > width) {
        y += 52;
