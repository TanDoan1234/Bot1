...
import {
  handleOnChatUser,
  handleOnReplyFromUser,
} from "../service-dqt/service.js";
...
export async function checkAndSendBusinessCard(api, senderId, senderName) {
  ...
  if (!lastSentTime || currentTime - lastSentTime >= BUSINESS_CARD_COOLDOWN) {
    ...
    if (
      admins.length == 0 ||
      (admins.length == 1 && admins.includes(idBot.toString()))
    )
      return false;
    await api.sendMessage(
      {
        msg:
          `Chào ${senderName}!, cảm ơn bạn đã liên hệ TD Shop MMO 👋\n` +
          `⏳ Nếu mình chưa kịp phản hồi, bạn có thể tham gia nhóm Zalo để được hỗ trợ nhanh chóng từ admin nhé:\n` +
          `\n` +
          `👉 🔗 Tham gia nhóm hỗ trợ:  https://zalo.me/g/hpfdju760\n` +
          `\n` +
          `🔹 Tài khoản Premium, AI, công cụ MMO\n` +
          `🔹 Các loại Tool, Key bản quyền, tài nguyên số uy tín\n` +
          `🔹 Hỗ trợ nhanh – giá tốt – bảo hành đầy đủ`,
      },
      senderId,
...
export async function messagesUser(api, message) {
  ...
  switch (message.type) {
    case MessageType.DirectMessage: {
      if (getConnectedClientsCount() > 0) {
        ...
        pushMessageToWebLog(
          ...
          senderName,
          content,
          userInfo.profiles[senderId].avatar
        );
      }
      if (contentText) {
      ...
      if (isPlainText) {
        ...
        continueProcessingChat =
          continueProcessingChat && (await canReplyToUser(senderId));
        continueProcessingChat =
          continueProcessingChat &&
          !(await handleOnReplyFromUser(api, message));
        if (continueProcessingChat) {
          const commandResult = await handleCommandPrivate(api, message);
          continueProcessingChat =
            continueProcessingChat && commandResult === 1 && !isSelf;
          continueProcessingChat =
            continueProcessingChat &&
            !(
              !isSelf &&
              (await checkAndSendBusinessCard(api, senderId, senderName))
            );
    ...
    case MessageType.GroupMessage: {
      ...
      handleChat =
        handleChat &&
        !(await superCheckBox(api, message, isSelf, botIsAdminBox, isAdminBox));
      ...
      if (isPlainText) {
        ...
        if (handleChat || (!isSelf && isAdminBot)) {
          await handleOnChatUser(
            api,
            message,
            numberHandleCommand === 5,
            groupSettings
          );
        }
