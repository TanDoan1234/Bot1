...
export function sendBusinessCardFactory(api) {
  const directMessageServiceURL = makeURL(
    `${api.zpwServiceMap.file[0]}/api/message/forward`,
    {
      zpw_ver: Zalo.API_VERSION,
      zpw_type: Zalo.API_TYPE,
    }
  );
  const groupMessageServiceURL = makeURL(
    `${api.zpwServiceMap.file[0]}/api/group/forward`,
    {
      zpw_ver: Zalo.API_VERSION,
      zpw_type: Zalo.API_TYPE,
    }
   ...
   * @throws {ZaloApiError}
   */
  return async function sendBusinessCard(
    message,
    userId,
    phone,
    type,
    ...
    if (!qrCodeUrl[userId.toString()])
      throw new ZaloApiError("QR Code URL not found");
    ...
    const serviceURL =
      type === MessageType.DirectMessage
        ? directMessageServiceURL
        : groupMessageServiceURL;
