...
export async function helpCommand(api, message, groupAdmins) {
  ...
  helpMessage += `║ 📅 ${prefix}plan [nội dung] [dd/mm/yyyy hh:mm] @user - Giao việc, nhắc nhở tự động\n`;
  helpMessage += `║ ✅ ${prefix}plan done [id] - Đánh dấu hoàn thành plan\n`;
  helpMessage += `║ 📋 ${prefix}plan list - Xem danh sách plan của nhóm\n`;
  helpMessage += "╚═════════════\n\n";
  ...
  let helpCommand = {
    ...
    allMembers: {
      ...
      // Thêm các lệnh !plan
      plan: {
        command: `${prefix}plan [nội dung] [dd/mm/yyyy hh:mm] @user`,
        description: "Giao việc, nhắc nhở tự động",
        icon: "📅",
      },
      plandone: {
        command: `${prefix}plan done [id]`,
        description: "Đánh dấu hoàn thành plan",
        icon: "✅",
      },
      planlist: {
        command: `${prefix}plan list`,
        description: "Xem danh sách plan của nhóm",
        icon: "📋",
      },
      image: {
  ...
  // Thêm tự động lệnh !update nếu chưa có
  if (!helpCommand.allMembers.update) {
    helpCommand.allMembers.update = {
    ...
    async default() {
      ...
      responseMsg += [
        ...
        `ℹ️ Dùng ${prefix}cmd map ${isAdminRequest ? "admin " : ""}` +
          `để xem toàn bộ lệnh dành cho ${
            isAdminRequest ? "admin" : "thành viên"
          }.`,
      ].join("\n");
