...
import {
  ...
  handleMuteUser,
  handleUnmuteUser,
} from "../service-dqt/anti-service/mute-user.js";
...
import {
  sendGifLocal,
  sendGifRemote,
} from "../service-dqt/chat-zalo/chat-special/send-gif/send-gif.js";
...
import {
  handleLearnCommand,
  handleReplyCommand,
} from "../service-dqt/chat-bot/bot-learning/dqt-bot.js";
...
} from "../service-dqt/api-crawl/music/zingmp3.js";
...
export async function handleCommand(
  ...
  if (typeof content === "string") {
    ...
    switch (command) {
      ...
      case "update":
        await handleUpdateCommand(api, message);
        break;

      case "plan":
        if (commandParts[1] === "done") {
          await handlePlanDoneCommand(api, message);
        } else if (commandParts[1] === "list") {
          await handlePlanListCommand(api, message, groupInfo);
        } else {
          await handlePlanCommand(api, message, groupInfo);
        }
        break;
