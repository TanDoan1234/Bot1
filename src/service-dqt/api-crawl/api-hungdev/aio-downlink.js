...
export const API_URL_DOWNAIO_HUNGDEV =
  "https://api.hungdev.id.vn/medias/down-aio";
...
const getMediaType = (url) => {
  ...
  return (
    Object.entries(MEDIA_TYPES).find(([domain]) =>
...
export const getDataDownloadVideo = async (url) => {
  try {
    const response = await axios.get(
      `${API_URL_DOWNAIO_HUNGDEV}?apikey=${API_KEY_HUNGDEV}&url=${encodeURIComponent(
        url
      )}`,
      {
        headers: {
...
export async function processAndSendMedia(api, message, mediaData) {
  ...
  if (typeFile === "image") {
    const thumbnailPath = path.resolve(
      tempDir,
      `${uniqueId}.${selectedMedia.extension}`
    );
  ...
  if ((mediaType === "youtube" || mediaType === "instagram") && duration) {
    if (duration * 1000 > 60 * 60 * 1000) {
      const object = {
        caption:
          "Vì tài nguyên có hạn, Không thể lấy video có độ dài hơn 60 phút!\nVui lòng chọn video khác.",
      };
...
export async function handleDownloadCommand(api, message, aliasCommand) {
  ...
  try {
    ...
    switch (mediaType) {
      case "tiktok":
        uniqueId = dataDownload.title.replace(/#\w+/g, (match) =>
          match.toLowerCase()
        );
    ...
    listText += dataLink
      .map(
        (item, index) =>
          `${index + 1}. ${item.type} - ${item.quality || "Unknown"} (${
            item.extension
          })`
      )
      .join("\n");
...
export async function categoryDownload(
  api,
  message,
  platform,
  uniqueId,
  ...
  try {
    switch (platform) {
      case "youtube":
        ...
        tempFilePath = await downloadYoutubeVideo(
          selectedMedia.url,
          uniqueId,
          format
        );
      ...
      default:
        ...
        tempFilePath = path.join(
          tempDir,
          `${platform}_${Date.now()}.${selectedMedia.extension}`
        );
    ...
    setCacheData(
      platform,
      uniqueId,
      { fileUrl: videoUrl, title: selectedMedia.title, duration },
      qualityVideo
    );
    return videoUrl;
  } catch (error) {
